...
@WebServlet("/InsertForumComment")
public class InsertForumComment extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String memberNo = "1";
            String forumCommentText = request.getParameter("forumComment");

            LocalDateTime now = LocalDateTime.now();
	        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
	        String forumDate = now.format(formatter);
	        
	        forumComment.setMemberNo(memberNo);
	        forumComment.setForumNo(forumNo);
	        forumComment.setForumComment(forumCommentText);
	        forumComment.setForumCommentDate(forumDate);

            forumCommentDao.insertComment(conn, forumComment);
